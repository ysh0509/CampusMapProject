 ...
 * [CASE 1] 실외 노드 간 이동: 위경도 좌표를 사용하여 실제 지구상의 거리를 추정함.
 * [CASE 2] 실내 노드 간 이동: 평면 좌표를 사용하여 2D 유클리드 거리를 계산함.
 * [CASE 3] 실내 ↔ 실외 이동: 'transfer_edge'로 연결된 구간은 휴리스틱 값을 0으로 설정하여 알고리즘이 연결 노드를 우선적으로 탐색하게 유도함.
 * 실외 노드 간 이동에서는 위경도 좌표의 차이를 실제 거리로 보정하기 위해 100,000을 곱하여 스케일링함.
 * 실내 노드 간 이동에서는 평면 좌표의 차이를 그대로 사용하여 2D 유클리드 거리를 계산함.
 * 실내와 실외를 연결하는 구간은 휴리스틱 값을 0으로 설정하여 알고리즘이 연결 노드를 우선적으로 탐색하게 유도함으로써, 실내와 실외 간의 자연스러운 이동을 지원함.
 * 노드 정보가 없는 경우에는 계산이 불가능하므로 휴리스틱 값을 0으로 반환하여 탐색이 계속 진행될 수 있도록 함.
 * 이 함수는 A* 알고리즘의 핵심 요소로, 탐색의 효율성과 정확성에 큰 영향을 미치므로, 노드 유형과 좌표 체계에 따라 적절한 거리 계산 방식을 적용하여 최적의 경로 탐색을 지원함.
 * 
 * 가중치 계산 로직(COST_MODE)은 탐색 모드에 따라 엣지의 비용을 다르게 계산하여 다양한 사용자 요구에 맞는 경로를 제공함.
 * 계단은 2, 오르막인 경사의 경우 3배, 내리막인 경사의 경우 1.2배 에스컬레이터는 1.5배의 가중치를 부여하여 사용자가 편한 길을 선호하도록 유도함.
 * 
 * 계단 회피 모드에서는 계단이나 에스컬레이터인 경우 비용을 무한대로 설정하여 탐색 대상에서 제외함으로써, 사용자가 계단을 피하는 경로를 우선적으로 탐색할 수 있도록 지원함.
 * 최적 경로 모드에서는 거리, 경사, 이동수단, 실시간 혼잡도를 종합적으로 고려하여 '가장 효율적인' 경로를 찾음으로써, 사용자가 편안하고 빠르게 이동할 수 있는 경로를 제공함.
 * 최단 시간 모드에서는 물리적인 시간(초 단위)을 계산하여 가장 '빨리 도착하는' 경로를 찾음으로써, 사용자가 시간 효율적인 경로를 선택할 수 있도록 지원함.
 * 
